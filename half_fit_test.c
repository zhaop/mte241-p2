...
__inline TimerInit(){
  ...
  while (SysTick_Config(SystemCoreClock / 1000 / 100))  {                                   
...
int main( void ) {
	...
	TimerStart(); {
		t1 = test_max_alc();
		t2 = test_alc_free_max();
		t3 = test_static_alc_free();
		t4 = test_static_alc_free_violation();
		t5 = test_rndm_alc_free();
		t6 = test_max_alc_1_byte();
	} TimerStop();
	...
	printf( "The elappsed time is %d0 us\n", current_elapsed_time() );
