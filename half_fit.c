...
#define BLOCK_SIZE 8			// Block size in U32 (type of memory array)
...
static U32 memory[32768/4] __attribute__ ((section(".ARM.__at_0x10000000"), zero_init));
...
__inline static void getBlockHeader(block_t* b, U16 v_addr) {
	...
	b->prev_buck =   h >> 22;
	b->next_buck =  (h >> 12) & LAST10;
}

__inline static U16 getNextBuck(U16 v_addr) {
	return (memory[v_addr*BLOCK_SIZE + 1] >> 12) & LAST10;
}
...
__inline static void setPrevBuck(U16 v_addr, U16 value) {
	memory[v_addr*BLOCK_SIZE + 1] = memory[v_addr*BLOCK_SIZE + 1]
		& ~PREV
		| value << 22;
}
